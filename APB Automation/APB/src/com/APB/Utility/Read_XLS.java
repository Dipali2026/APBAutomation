...
public class Read_XLS {	
	...
	public int retrieveNoOfCols1(String wsName){
		int sheetIndex=wb.getSheetIndex(wsName);
		if(sheetIndex==-1)
			return 0;
		else{
	...
	//To write result In test data and test case list sheet.
	public boolean writePayTransferResult(String wsName,String colName,int rowNumber,String Result){
		try{
			int sheetIndex=wb.getSheetIndex(wsName);
			if(sheetIndex==-1)
				return false;			
			int colNum = retrieveNoOfCols1(wsName);
			int colNumber=-1;
					
			
			HSSFRow Suiterow = ws.getRow(1);			
			for(int i=0; i<colNum; i++){				
				if(Suiterow.getCell(i).getStringCellValue().equals(colName.trim())){
					colNumber=i;					
				}					
			...
			HSSFRow Row = ws.getRow(rowNumber1);
			HSSFCell cell = Row.getCell(colNumber);
			if (cell == null)
		        cell = Row.createCell(colNumber);			
	...
	//To write result In test data and test case list sheet.
		public boolean writePayRefundResult(String wsName,String colName,int rowNumber,String Result){
			try{
				int sheetIndex=wb.getSheetIndex(wsName);
				if(sheetIndex==-1)
					return false;			
		...
		//To write result In test suite client
		public boolean writeClientResult(String wsName, String colName,String Result){
			try{
				int rowNum = retrieveNoOfRows(wsName);
				int rowNumber=-1;
				int sheetIndex=wb.getSheetIndex(wsName);
